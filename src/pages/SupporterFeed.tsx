import { CalendarDays, Filter, RefreshCw, Share2, Shield, Trophy, UserRound } from 'lucide-react'
...
function FeedCard({ item }: { item: FeedItem }) {
  const [sharing, setSharing] = useState<'idle' | 'creating' | 'copied'>('idle')
  ...
  const share = async () => {
    ...
    setSharing('creating')
    try {
      const result = await sharePlayFootyPath(item.href)
      setSharing(result === 'copied' ? 'copied' : 'idle')
      if (result === 'copied') window.setTimeout(() => setSharing('idle'), 1800)
    } catch {
      setSharing('idle')
    }
  }
  return <article className="sf-card">
    <Link to={item.href} className="sf-card-link" onClick={() => markRead(item.id)} aria-label={`View ${item.title}`}>
    ...
    <button type="button" className="sf-share" onClick={() => void share()} disabled={sharing === 'creating'} aria-label={`Share ${item.title}`}><Share2 size={16}/><span>{sharing === 'creating' ? 'Creating' : sharing === 'copied' ? 'Copied' : 'Share card'}</span></button>
  </article>
}
