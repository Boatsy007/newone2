...
export default function CoachAppWhiteboardStage2({clubId,sheetId,token,fixtureLabel,returnToMatch,onExit}:Props){
  ...
  const[playerArea,setPlayerArea]=useState<'ALL'|'FWD'|'MID'|'DEF'>('ALL')
  ...
  const selectedMagnet=state.magnets.find(item=>item.id===selectedMagnetId)||null
...
.wb-topbar{height:64px!important;min-height:64px!important;padding:8px 14px!important;background:rgba(5,20,29,.97)!important;border-bottom:1px solid #173844!important;display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:center!important;gap:14px!important;z-index:40!important}
