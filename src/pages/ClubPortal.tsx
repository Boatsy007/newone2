...
export default function ClubPortal() {
  ...
  const [message, setMessage] = useState(portalReasonMessage())
  ...
  useEffect(() => {
    ...
    const candidate = confirmed ?? stored
    if (!candidate) return
    void ensureFreshPortalSession(candidate).then(current => {
      if (!active || !current?.club_accounts?.length) {
        if (!confirmed && active) saveSession(null)
        if (confirmed && active) setMessage('Your email is confirmed. Sign in to open your club.')
        return
      }
      saveSession(current)
      setSession(current)
      setAccounts(current.club_accounts ?? [])
      const returnTo = safePortalReturnTo('club')
      if (returnTo) window.location.replace(returnTo)
      else if (current.club_accounts?.length === 1) openClub(current.club_accounts[0].clubId)
    }).catch(() => {
      if (active) { saveSession(null); setMessage('Your previous session expired. Sign in again to continue.') }
    })
    return () => { active = false }
  }, [])
  ...
  async function submitAuth(event: React.FormEvent) {
    ...
    try {
      ...
      if (mode === 'signup' && !result.access_token) { setMessage('Account created. Confirm your email, then sign in to open your club.'); setMode('signin'); setPassword(''); return }
      const linked = result.club_accounts ?? []
      if (!linked.length) { saveSession(null); setSession(null); setError('This login is not linked to an active club. Ask the club Owner or PlayFooty Admin to invite or approve you.'); return }
      saveSession(result); setSession(result); setAccounts(linked); setPassword('')
      const returnTo = safePortalReturnTo('club')
      if (returnTo) window.location.replace(returnTo)
      else if (linked.length === 1) openClub(linked[0].clubId)
  ...
  async function forgotPassword() {
    ...
    try { await requestPortalPasswordReset(email, '/club-portal'); setMessage('Password reset email sent. Check your inbox and junk folder.') }
