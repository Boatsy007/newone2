...
function HighlightEditor({row,update,winner,remove}:{row:Row;update:(id:string,body:Record<string,unknown>)=>Promise<void>;winner:(id:string)=>Promise<void>;remove:(id:string)=>Promise<void>}){
  ...
  const save=()=>update(row.id,{playerId,clubId,leagueId,matchId,headline,description,articleBody,videoUrl,thumbnailUrl,moderationNote:note,votingOpensAt:opens?new Date(opens).toISOString():null,votingClosesAt:closes?new Date(closes).toISOString():null})
