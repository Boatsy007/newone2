...
export default function CoachAppTrainingReport({clubId,token,sessionNumber,fixtureDate,onExit,onContinue}:Props){
 ...
 function dropPlayer(performance:Performance){const id=dragPlayer||selectedPlayer;if(id)movePlayer(id,performance);setDragPlayer('')}
 ...
 return <section className="catr"><style>{styles}</style>
  ...
  <section className="catr-card"><div className="catr-heading"><div><span>Drag or tap</span><h2>How did everyone train?</h2></div><button onClick={()=>{const next:Record<string,PlayerEntry>={};for(const player of players)next[player.id]={...entries[player.id]!,status:'ATTENDED',performance:'SOLID'};setEntries(next)}}><Check/>Mark all solid</button></div>
   <div className="catr-groups">{performanceGroups.map(group=>{const Icon=group.icon;const groupPlayers=players.filter(player=>entries[player.id]?.performance===group.key);return <section key={group.key} className={`catr-group ${group.key.toLowerCase()}`} onDragOver={event=>event.preventDefault()} onDrop={()=>dropPlayer(group.key)} onClick={()=>{if(selectedPlayer)dropPlayer(group.key)}}><header><Icon/><div><h3>{group.label}</h3><small>{group.help}</small></div><b>{groupPlayers.length}</b></header><div>{groupPlayers.map(player=>{const entry=entries[player.id]!;return <button key={player.id} draggable onDragStart={()=>setDragPlayer(player.id)} onClick={event=>{event.stopPropagation();setSelectedPlayer(current=>current===player.id?'':player.id)}} className={`${selectedPlayer===player.id?'selected':''} ${entry.status==='INJURED'?'injured':''}`}><i>{player.jumperNumber??'–'}</i><span><strong>{player.playerName}</strong><small>{entry.status.replace('_',' ')}{entry.injuryDetail?` · ${entry.injuryDetail}`:''}</small></span><em onClick={event=>{event.stopPropagation();setOpenPlayer(player.id)}}>Edit</em></button>})}</div></section>})}</div>
  </section>
