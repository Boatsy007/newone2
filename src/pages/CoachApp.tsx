...
type ToolKey = 'training-plan' | 'training-summary' | 'availability' | 'game-plan' | 'match-day'
...
const flow: Array<{key:ToolKey;label:string;eyebrow:string}> = [
  ...
  {key:'training-summary',label:'Training Summary',eyebrow:'Session 1'},
  {key:'availability',label:'This Week Availability',eyebrow:'Players'},
  {key:'training-plan',label:'Training Plan',eyebrow:'Session 2'},
  {key:'training-summary',label:'Training Summary',eyebrow:'Session 2'},
...
export default function CoachApp(){
  ...
  return <main className="coach-app-root"><style>{styles}</style>
    ...
    {screen==='DASHBOARD'&&<section className="coach-dashboard">
      ...
      <section className="match-flow"><div className="section-heading"><div><span>Current week</span><h2>Match Flow</h2></div><small>{fixtureLabel}</small></div><div className="flow-track">{flow.map((item,index)=>{const sessionNumber=index<2?1:index>2&&index<5?2:undefined;return <button key={`${item.key}-${index}`} className={`flow-card ${item.key}`} onClick={()=>openTool(item.key,sessionNumber)}><i>{index+1}</i><span>{item.eyebrow}</span><strong>{item.label}</strong><em>{index<flow.length-1?'Open step':'Start match day'}</em></button>})}</div></section>
      <section className="coach-tools"><div className="section-heading"><div><span>Open directly</span><h2>Coach Tools</h2></div><small>One card for each tool</small></div><div className="tool-grid">
        ...
        <button onClick={()=>openTool('training-summary')}><ClipboardCheck/><span><b>Training Summary</b><small>Complete the report after training.</small></span></button>
