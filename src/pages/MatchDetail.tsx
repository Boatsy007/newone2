...
export default function MatchDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    const importedFootball = matchId.startsWith('football:')
    const sourceId = importedFootball ? matchId.slice('football:'.length) : matchId
    const collection = safeKind === 'result' ? 'results' : 'fixtures'
    const url = importedFootball
      ? `/api/${collection}/football/${encodeURIComponent(sourceId)}`
      : `/api/${collection}/${encodeURIComponent(sourceId)}`
    ...
    {!loading && data && <>
      ...
      <section className="md-shell md-grid">
        <article className="md-card"><header><span>Match information</span><h2>Game details</h2></header><dl>
          ...
          <Info label="State" value={data.state ?? 'Not provided'} />
