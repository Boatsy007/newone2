  ...
  return <div className="md-page"><Nav /><main>
    ...
    {!loading && data && <>
      <section className="md-hero"><div className="md-shell">
        ...
        <div className="md-scoreboard"><TeamBlock id={data.homeClubId} name={home} /><ScoreBlock data={data} result={safeKind === 'result'} /><TeamBlock id={data.awayClubId} name={away} /></div>
      </div></section>
...
function TeamBlock({ id, name }: { id?: string | null; name: string }) { const initials = name.split(/\s+/).filter(Boolean).slice(0,2).map(part => part[0]).join('').toUpperCase() || 'FC'; const content = <><span>{initials}</span><strong>{name}</strong></>; return id ? <Link to={`/team/${id}`} className="md-team">{content}</Link> : <div className="md-team">{content}</div> }
