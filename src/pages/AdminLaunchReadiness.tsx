...
export default function AdminLaunchReadiness() {
  const [manual, setManual] = useState<Record<string, boolean>>(() => { try { return JSON.parse(localStorage.getItem(STORAGE) ?? '{}') as Record<string, boolean> } catch { return {} } })
  ...
  const toggle = (id: string) => {
    ...
    setManual(next); localStorage.setItem(STORAGE, JSON.stringify(next))
  }
  ...
  return <main className="launch-page"><style>{styles}</style>
    ...
    <section className="launch-list">{checks.map(check => {
      ...
      return <article key={check.id} className={state === 'fail' ? 'failed' : ''}>
        <button type="button" onClick={() => !check.automatic && toggle(check.id)} disabled={check.automatic} aria-label={`${check.label}: ${state}`}>
          {state === 'pass' ? <CheckCircle2/> : state === 'checking' ? <RefreshCw className="spin"/> : <Circle/>}
        </button>
        <div><strong>{check.label}</strong><p>{check.detail}</p><small>{check.automatic ? `Automated · ${state === 'checking' ? 'checking' : state === 'pass' ? 'passed' : state === 'fail' ? 'failed' : 'not run'}` : 'Manual owner confirmation · saved in this browser'}</small></div>
...
const styles = `.launch-page,.launch-login{min-height:100vh;background:#eef2f6;color:#111318;font-family:Barlow,Inter,Arial,sans-serif}.launch-page>header{position:sticky;top:0;z-index:10;height:70px;padding:0 clamp(14px,4vw,38px);display:flex;align-items:center;justify-content:space-between;background:#fff;border-bottom:1px solid #dce3eb}.launch-page header a,.launch-page header button,.launch-login a{display:inline-flex;align-items:center;gap:8px;border:0;border-radius:999px;padding:11px 16px;text-decoration:none;font-weight:950;text-transform:uppercase}.launch-page header a{background:#eef2f6;color:#111318}.launch-page header button,.launch-login a{background:#35b6ff;color:#050505;cursor:pointer}.launch-hero,.launch-summary,.launch-list{width:min(1120px,calc(100% - 28px));margin:18px auto 0}.launch-hero{padding:clamp(32px,7vw,62px);border-radius:23px;background:#050505;color:#fff}.launch-hero>span{color:#35b6ff;font-size:11px;font-weight:950;letter-spacing:.17em}.launch-hero h1{margin:9px 0 12px;font-family:'Bebas Neue',Impact,sans-serif;font-size:clamp(4rem,10vw,7.5rem);line-height:.82;text-transform:uppercase}.launch-hero p{max-width:760px;margin:0;color:#cbd3dd;font-size:17px;line-height:1.55}.launch-summary{display:grid;grid-template-columns:1fr 1fr;gap:13px}.launch-summary article{display:grid;grid-template-columns:auto 1fr;align-items:center;gap:10px;padding:19px;border:1px solid #dce3eb;border-radius:17px;background:#fff}.launch-summary svg{grid-row:1/3;color:#128a4a}.launch-summary span{color:#687385;font-size:10px;font-weight:950;text-transform:uppercase}.launch-summary strong{font-size:30px}.launch-summary .blocked svg,.launch-summary .blocked strong{color:#d71920}.launch-list{display:grid;gap:10px;padding-bottom:60px}.launch-list article{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:14px;padding:17px;border:1px solid #dce3eb;border-radius:16px;background:#fff}.launch-list article.failed{border-color:#e8aeb1;background:#fff6f6}.launch-list article>button{display:grid;place-items:center;width:42px;height:42px;border:0;border-radius:50%;background:#eef8ff;color:#0783c9;cursor:pointer}.launch-list article>button:disabled{cursor:default}.launch-list article>div strong{font-size:17px}.launch-list article p{margin:4px 0;color:#586370;line-height:1.45}.launch-list article small{color:#84909e;font-weight:800}.launch-list article>a{color:#0783c9}.launch-login{display:grid;place-items:center;padding:20px}.launch-login section{width:min(480px,100%);padding:34px;border-radius:20px;background:#050505;color:#fff;text-align:center}.launch-login h1{font-family:'Bebas Neue',Impact,sans-serif;font-size:3.2rem;text-transform:uppercase}.launch-login p{color:#cbd3dd;line-height:1.5}.spin{animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:650px){.launch-summary{grid-template-columns:1fr}.launch-list article{grid-template-columns:auto 1fr}.launch-list article>a{grid-column:2}.launch-page>header{gap:8px}.launch-page header a,.launch-page header button{padding:9px 11px;font-size:11px}}`
