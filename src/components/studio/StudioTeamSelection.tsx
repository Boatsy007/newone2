...
import { ArrowLeft, CheckCircle2, Clipboard, Download, Image as ImageIcon, LoaderCircle, LockKeyhole, RefreshCw, Share2 } from 'lucide-react'
...
function fileName(club:Club,context:Context|null){return `${club.name}-${context?.fixture?.round||'team-selection'}.png`.toLowerCase().replace(/[^a-z0-9.-]+/g,'-')}
...
export default function StudioTeamSelection({clubId,token,club,onBack}:Props){
 ...
 async function generate(){if(!ready||!context||!sheet||!canvas.current)return;setBusy(true);setError('');setMessage('');setAssetId('');try{const ctx=canvas.current.getContext('2d');if(!ctx)throw new Error('Canvas is unavailable');const W=1080,H=1350,primary=safeColour(primaryColour,'#082f6d'),secondary=safeColour(accentColour,'#42b8ff');canvas.current.width=W;canvas.current.height=H
   ...
   ctx.fillStyle='#fff';ctx.font='700 30px Arial';ctx.textAlign='right';ctx.fillText((context.fixture?.round||sheet.roundLabel||'MATCH WEEK').toUpperCase(),1005,95);ctx.font='900 34px Arial';ctx.fillText(`V ${opp||sheet.opponentName||'OPPONENT TBC'}`.toUpperCase(),1005,143);ctx.fillStyle='rgba(255,255,255,.82)';ctx.font='700 19px Arial';ctx.fillText(`${dateLabel(context.fixture?.matchDate||sheet.matchDate)} · ${(context.fixture?.venue||'GROUND TBC').toUpperCase()}`,1005,178);ctx.textAlign='left'
 ...
 function download(){if(!imageUrl)return;const anchor=document.createElement('a');anchor.download=fileName(club,context);anchor.href=imageUrl;anchor.click()}
 async function saveToDevice(){if(!imageUrl)return;setError('');try{const response=await fetch(imageUrl);const blob=await response.blob();const file=new File([blob],fileName(club,context),{type:'image/png'});if(typeof navigator.share==='function'&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({files:[file],title:`${club.name} Team Selection`});setMessage('Image opened in your device share sheet — choose Save Image or Save to Files.');return}download();setMessage('Image download started.')}catch(reason){if(reason instanceof DOMException&&reason.name==='AbortError')return;download();setMessage('Image download started.')}}
 ...
 return <section className="sts">
  ...
  {imageUrl&&<section className="sts-package"><div className="sts-preview"><header><span>IMAGE</span><strong>1080 × 1350</strong></header><img src={imageUrl} alt={`${club.name} team selection graphic`}/><button onClick={()=>void saveToDevice()}><Share2/> Save to phone / iPad</button></div><div className="sts-captions"><header><span>CAPTIONS</span><strong>Ready to post</strong></header><label>Primary social caption<textarea value={caption} onChange={event=>setCaption(event.target.value)}/><button onClick={()=>void copy(caption,'Caption')}><Clipboard/> Copy caption</button></label><label>Short caption<textarea value={compactCaption} onChange={event=>setCompactCaption(event.target.value)}/><button onClick={()=>void copy(compactCaption,'Short caption')}><Clipboard/> Copy short caption</button></label>{assetId&&<small>Saved to the existing PlayFooty Media Centre as asset {assetId.slice(0,8)}…</small>}</div></section>}
