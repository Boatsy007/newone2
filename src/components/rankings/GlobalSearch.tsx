...
type GoalKicker = { rank: number; playerName: string; goals: number; clubName: string; leagueName: string; clubLogoUrl?: string | null }
type Destination = { label: string; description: string; path: string; keywords: string }
...
export default function GlobalSearch({ controller }: { controller: SearchController }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/goal-kickers?mode=raw&limit=250').then(response => response.ok ? response.json() : Promise.reject(new Error(`HTTP ${response.status}`))).then((payload: { data?: GoalKicker[] }) => setGoalKickers(Array.isArray(payload.data) ? payload.data : [])).catch(() => setGoalKickers([]))
    fetchRankings().then(response => setRankings(response.data)).catch(() => setRankings([]))
  }, [])

  useEffect(() => {
    if (!active) { setResults({ teams: [], leagues: [] }); setLoading(false); return }
    setLoading(true)
    const timer = window.setTimeout(() => {
      fetchSearch(query.trim()).then(setResults).catch(() => setResults({ teams: [], leagues: [] })).finally(() => setLoading(false))
    }, 220)
  ...
  return <div className="pf-global-search" onMouseDown={event => { if (event.target === event.currentTarget) close() }}>
    <section className="pf-search-dialog" role="dialog" aria-modal="true" aria-label="Search PlayFooty">
      <header className="pf-search-header"><div><span>Universal search</span><h2>Find anything in PlayFooty</h2><p>Search clubs, leagues, players, goal kickers, news and site sections.</p></div><button type="button" onClick={close} aria-label="Close search"><X size={22} /></button></header>
      <label className="pf-search-field"><Search size={23} /><input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} placeholder="Search a club, league, player or story" autoComplete="off" /><kbd>ESC</kbd></label>
      <div className="pf-search-content">
        ...
        {active && results.teams.length > 0 && <ResultGroup title="Clubs & teams">{results.teams.map(team => <button type="button" key={team.clubId} className="pf-search-result" onClick={() => go(teamPath(team.clubId))}><TeamLogo name={team.clubName} src={logoByClubId.get(team.clubId)} size={48} /><span><strong>{team.clubName}</strong><small>{team.leagueName} · {team.state}</small></span><em>#{team.rank}</em></button>)}</ResultGroup>}
        {active && players.length > 0 && <ResultGroup title="Players & goal kickers">{players.map(player => <button type="button" key={`${player.rank}-${player.playerName}-${player.clubName}`} className="pf-search-result" onClick={() => go(`/goal-kickers?q=${encodeURIComponent(player.playerName)}`)}><TeamLogo name={player.clubName} src={player.clubLogoUrl ?? undefined} size={48} /><span><strong>{player.playerName}</strong><small>{player.clubName} · {player.leagueName}</small></span><em>{player.goals} goals</em></button>)}</ResultGroup>}
        {active && results.leagues.length > 0 && <ResultGroup title="Leagues">{results.leagues.map(league => <button type="button" key={league.id} className="pf-search-result" onClick={() => go(leaguePath(league.id))}><span className="pf-league-mark">{league.state}</span><span><strong>{league.name}</strong><small>{league.state} · League profile and ladder</small></span><StarStrength stars={strengthStars(league.strengthScore)} size={10} /></button>)}</ResultGroup>}
        {active && news.length > 0 && <ResultGroup title="News">{news.map(article => <NewsResult key={article.slug} article={article} go={go} />)}</ResultGroup>}
        {active && pages.length > 0 && <ResultGroup title="PlayFooty sections">{pages.map(page => <button type="button" key={page.path} className="pf-search-result pf-page-result" onClick={() => go(page.path)}><span className="pf-page-icon"><Trophy size={20} /></span><span><strong>{page.label}</strong><small>{page.description}</small></span><ArrowRight size={18} /></button>)}</ResultGroup>}
      </div>
    </section>
    <SearchStyles />
  </div>
...
function NewsResult({ article, go }: { article: Article; go: (path: string) => void }) { const category = categoryOf(article.category); return <button type="button" className="pf-search-result pf-news-result" onClick={() => go(newsPath(article.slug))}><EditorialImage seed={article.heroSeed} ratio="4 / 3" rounded={6} label={category.label} /><span><i style={{ background: category.accent }}>{category.label}</i><strong>{article.title}</strong><small>{formatDate(article.date)} · {article.readingTime} min read</small></span><ArrowRight size={18} /></button> }
function SearchStart({ suggestions, pages, go }: { suggestions: Article[]; pages: Destination[]; go: (path: string) => void }) { return <div className="pf-search-start"><div><span className="pf-search-eyebrow">Popular destinations</span><div className="pf-search-shortcuts">{pages.map(page => <button type="button" key={page.path} onClick={() => go(page.path)}><strong>{page.label}</strong><small>{page.description}</small><ArrowRight size={16} /></button>)}</div></div>{suggestions.length > 0 && <div><span className="pf-search-eyebrow">Latest stories</span><div className="pf-search-suggestions">{suggestions.map(article => <button type="button" key={article.slug} onClick={() => go(newsPath(article.slug))}>{article.title}<ArrowRight size={14} /></button>)}</div></div>}</div> }
function NoResults({ query }: { query: string }) { return <div className="pf-search-status"><strong>No matches for “{query.trim()}”</strong><span>Try a club, league, player, state, ranking or news topic.</span></div> }
...
.pf-search-status{display:grid;place-items:center;min-height:150px;padding:25px;text-align:center;color:${MUTED};font-weight:800}.pf-search-status strong{color:${TEXT};font-size:20px}.pf-search-status span{margin-top:6px;font-size:13px;font-weight:500}.pf-search-result:focus-visible,.pf-search-header button:focus-visible,.pf-search-shortcuts button:focus-visible,.pf-search-suggestions button:focus-visible{outline:3px solid rgba(45,170,245,.35);outline-offset:2px}
