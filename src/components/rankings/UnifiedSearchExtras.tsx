...
import { CalendarDays, FileText, Search, Shield, Trophy, Users, Video } from 'lucide-react'
import { useNavigate } from 'react-router-dom'

type SearchData = {
  clubs: { id: string; name: string; state: string; leagueName?: string | null; aliases?: string[]; href: string }[]
  leagues: { id: string; name: string; state: string; strengthScore?: number | null; href: string }[]
  ...
  teamSelections: { id: string; clubName: string; leagueName?: string | null; roundLabel: string; opponentName?: string | null; matchDate?: string | null; title: string; href: string }[]
...
const EMPTY: SearchData = { clubs: [], leagues: [], players: [], matches: [], teamSelections: [], news: [], highlights: [], records: [] }
...
export default function UnifiedSearchExtras() {
        ...
        fetch(`/api/search?q=${encodeURIComponent(nextQuery)}`, { signal: controller.signal })
          ...
          .then((payload: SearchPayload) => {
            ...
            setData({
              ...
              teamSelections: value.teamSelections ?? [],
              news: value.news ?? [],
  ...
  const hasExtras = aliasClubs.length + data.leagues.length + data.players.length + data.matches.length + data.teamSelections.length + data.news.length + data.highlights.length + data.records.length > 0
  ...
  return createPortal(<>
    ...
    {data.leagues.length > 0 && <Group title="Leagues">{data.leagues.map(row => <Row key={row.id} title={row.name} detail={`${row.state} community football`} metric={row.strengthScore != null ? row.strengthScore.toFixed(1) : undefined} icon={<Shield size={19}/>} onClick={() => go(row.href)}/>)}</Group>}
    ...
    {data.teamSelections.length > 0 && <Group title="Published teams">{data.teamSelections.map(row => <Row key={row.id} title={row.title} detail={`${row.roundLabel}${row.opponentName ? ` · v ${row.opponentName}` : ''}${row.matchDate ? ` · ${dateLabel(row.matchDate)}` : ''}${row.leagueName ? ` · ${row.leagueName}` : ''}`} metric="Team sheet" icon={<Users size={19}/>} onClick={() => go(row.href)}/>)}</Group>}
    ...
    {data.news.length > 0 && <Group title="Published news">{data.news.map(row => <Row key={row.id} title={row.title} detail={row.summary} metric={row.category} icon={<FileText size={19}/>} onClick={() => go(row.href)}/>)}</Group>}
