import { Activity, CalendarClock, Film, Layers3, Swords } from 'lucide-react'
...
export default function AdminHealthLink() {
  ...
  return <>
    <div className="admin-ops-links">
      <Link to="/admin/highlights"><Film size={18}/><span>Highlights</span></Link>
      <Link to="/admin/featured-games"><Swords size={18}/><span>Featured Games</span></Link>
    ...
    <style>{`
      .admin-ops-links{position:fixed;right:18px;bottom:18px;z-index:95;display:flex;gap:9px;align-items:center;flex-wrap:wrap;justify-content:flex-end;max-width:min(900px,calc(100vw - 36px))}
      ...
      .admin-ops-links a:nth-child(4){background:#050505;color:#fff}
      .admin-ops-links a:hover,.admin-ops-links a:focus-visible{background:#fff;color:#050505;outline:3px solid rgba(53,182,255,.32)}
      @media(max-width:900px){.admin-ops-links{right:12px;bottom:12px;display:grid;max-height:58vh;overflow:auto}.admin-ops-links a{padding:10px 13px}.admin-ops-links span{font-size:10px}}
    `}</style>
  </>
}
