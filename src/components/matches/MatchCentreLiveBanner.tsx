...
import PublicClubLiveMatch from '../club/PublicClubLiveMatch'

type MatchRow={id:string;sourceId?:string|null;leagueName?:string|null;round?:string|number|null;matchDate?:string|null;matchTime?:string|null;homeClubId?:string|null;homeClubName?:string|null;homeName?:string|null;awayClubName?:string|null;awayName?:string|null;homeScore?:number|null;awayScore?:number|null;homePoints?:number|null;awayPoints?:number|null;status?:string|null;sourceType?:'generic'|'football'|'portal'}
type Payload={data?:MatchRow[]}

async function load(url:string,sourceType:MatchRow['sourceType']){
 try{const response=await fetch(url,{cache:'no-store'});if(!response.ok)return[];const payload=await response.json() as Payload;return(Array.isArray(payload.data)?payload.data:[]).map(row=>({...row,sourceType}))}catch{return[]}
}

export default function MatchCentreLiveBanner(){
 const{pathname}=useLocation()
 const[host,setHost]=useState<HTMLElement|null>(null)
 const[clubHost,setClubHost]=useState<HTMLElement|null>(null)
 const[rows,setRows]=useState<MatchRow[]>([])
 const clubId=pathname.match(/^\/team\/([^/?#]+)/)?.[1]??''
 ...
 useEffect(()=>{
  if(!clubId){setClubHost(null);return}
  let active=true
  const attach=()=>{
   const hub=document.querySelector('.club-live-hub')
   if(!hub)return false
   let node=document.getElementById('club-public-live-match-host')
   if(!node){node=document.createElement('div');node.id='club-public-live-match-host';hub.prepend(node)}
   if(active)setClubHost(node)
   return true
  }
  if(!attach()){
   const observer=new MutationObserver(()=>{if(attach())observer.disconnect()})
   observer.observe(document.body,{childList:true,subtree:true})
   return()=>{active=false;observer.disconnect();document.getElementById('club-public-live-match-host')?.remove()}
  }
  return()=>{active=false;document.getElementById('club-public-live-match-host')?.remove()}
 },[clubId])
 ...
 useEffect(()=>{
  ...
  const refresh=()=>Promise.all([
   load('/api/live-match','portal'),load('/api/fixtures','generic'),load('/api/fixtures/football?limit=1000','football'),
...
function LiveCard({row}:{row:MatchRow}){
 ...
 const href=row.sourceType==='portal'&&row.homeClubId?`/team/${encodeURIComponent(row.homeClubId)}`:`/match/result/${encodeURIComponent(rawId)}${football&&!String(rawId).startsWith('football:')?'?source=football':''}`
