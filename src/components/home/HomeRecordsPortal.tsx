...
type RecordCard = { key: string; label: string; value: string; title: string; detail: string; footer: string; url: string; logoUrl: string | null }
...
type PlayerLogoMap = Map<string, { clubId: string | null; clubName: string; logoUrl: string | null }>
...
export default function HomeRecordsPortal() {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      if (!active) return
      const [weekResult, seasonResult, playerResult, ladderResult, logoResult] = results
      ...
      const logoMaps = logoResult.status === 'fulfilled' ? logoResult.value : { byClubId: new Map<string, string>(), byPlayerId: new Map() as PlayerLogoMap }
      ...
      if (weekData) {
        const currentWeekCards = buildCards(weekData.categories, 'this week', weeklyBag, true, logoMaps.byClubId, logoMaps.byPlayerId)
        if (currentWeekCards.length > 0) {
          ...
          setWeekly(buildCards(latest.categories, 'in the latest week', weeklyBag, true, logoMaps.byClubId, logoMaps.byPlayerId))
      ...
      if (seasonData) setYearly(buildCards(seasonData.categories, 'this year', yearlyBag, true, logoMaps.byClubId, logoMaps.byPlayerId))
    })
  ...
  return createPortal(<>
    ...
    <style>{`
      ...
      .pf-records-home{padding:0 0 46px;font-family:Barlow,Inter,Arial,sans-serif}.pf-records-head{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:17px}.pf-records-head>div>span{text-transform:uppercase;font-size:10px;font-weight:900;letter-spacing:.16em;color:#0783c9}.pf-records-head h2{font-family:'Bebas Neue',Impact,sans-serif;text-transform:uppercase;font-size:clamp(2.2rem,4vw,4rem);line-height:.88;margin:5px 0 0}.pf-records-head>a{display:inline-flex;align-items:center;gap:8px;color:#42b8ff;text-decoration:none;text-transform:uppercase;font-size:12px;font-weight:800}.pf-records-strip{display:flex;gap:14px;overflow-x:auto;scroll-snap-type:x mandatory;scroll-behavior:smooth;overscroll-behavior-inline:contain;padding-bottom:8px;scrollbar-width:none}.pf-records-strip::-webkit-scrollbar{display:none}.pf-record-card{position:relative;flex:0 0 min(285px,80vw);scroll-snap-align:start;scroll-snap-stop:always;border:1px solid #e3e7ec;border-radius:9px;padding:20px;background:#fff;color:#111318;text-decoration:none;min-height:205px;display:flex;flex-direction:column;transition:transform .18s ease;box-shadow:0 5px 16px rgba(17,24,39,.045)}.pf-record-logo{position:absolute;right:18px;top:18px;display:grid;place-items:center;width:56px;height:56px}.pf-record-logo img{max-width:52px;max-height:52px;object-fit:contain}.pf-record-card>span:not(.pf-record-logo){max-width:calc(100% - 72px);text-transform:uppercase;font-size:10px;letter-spacing:.13em;font-weight:900;color:#0783c9}.pf-record-card>strong{font-family:'Bebas Neue',Impact,sans-serif;font-size:42px;line-height:1;margin-top:16px;color:#42b8ff}.pf-record-card h3{font-family:'Bebas Neue',Impact,sans-serif;text-transform:uppercase;font-size:25px;line-height:1;margin:13px 0 5px}.pf-record-card p{font-size:13px;line-height:1.45;margin:0;color:#303741}.pf-record-card small{margin-top:auto;padding-top:14px;color:#687385;font-size:11px}.pf-record-card:hover{transform:translateY(-2px)}
      .pf-records-home.is-weekly .pf-record-card{background:#42b8ff;color:#050505;border-color:#159fe9;box-shadow:0 12px 30px rgba(45,170,245,.16)}.pf-records-home.is-weekly .pf-record-card>span:not(.pf-record-logo){align-self:flex-start;background:#050505;color:#fff;border-radius:999px;padding:7px 10px;line-height:1}.pf-records-home.is-weekly .pf-record-card>strong{color:#050505}.pf-records-home.is-weekly .pf-record-card p{color:#fff;font-weight:600}.pf-records-home.is-weekly .pf-record-card small{color:rgba(5,5,5,.72);font-weight:700}.pf-records-home.is-weekly .pf-records-head>a{color:#0783c9}
      @media(max-width:620px){.pf-records-home{padding-bottom:38px}.pf-records-head{align-items:flex-end}.pf-records-head h2{font-size:2.8rem}.pf-records-head>a{font-size:11px}.pf-records-strip{gap:12px}.pf-record-card{flex-basis:100%}}
    `}</style>
  </>, target)
}
...
function buildCards(records: RecordCategories, periodLabel: string, playerBag: PlayerBag | undefined, playerFirst: boolean, clubLogos: Map<string, string>, playerLogos: PlayerLogoMap): RecordCard[] {
  const recordCards = categories.filter(category => !playerFirst || category !== 'closestMatch').flatMap(category => {
    ...
    return entry ? [{
      ...
      url: entry.matchUrl,
      logoUrl: entry.clubId ? clubLogos.get(entry.clubId) ?? null : null,
    }] : []
  })
  const resolvedPlayer = playerBag ? playerLogos.get(playerBag.playerId) : undefined
  const sameClub = Boolean(playerBag && resolvedPlayer?.clubName.trim().toLowerCase() === playerBag.clubName.trim().toLowerCase())
  const playerCard = playerBag?.weeklyGoals ? {
    ...
    logoUrl: sameClub ? resolvedPlayer?.logoUrl ?? null : null,
  } : null
...
function RecordSection({ title, eyebrow, cards, variant }: { title: string; eyebrow: string; cards: RecordCard[]; variant: RecordSectionVariant }) {
  ...
  return <section className={`pf-records-home pf-shell is-${variant}`}><div className="pf-records-head"><div><span>{eyebrow}</span><h2>{title}</h2></div><Link to="/records">View all records <ArrowRight size={17} /></Link></div><div className="pf-records-strip">{cards.map(card => <Link key={card.key} to={card.url} className="pf-record-card"><span className="pf-record-logo"><TeamLogo name={card.title} src={card.logoUrl ?? undefined} size={52} /></span><span>{card.label}</span><strong>{card.value}</strong><h3>{card.title}</h3><p>{card.detail}</p><small>{card.footer}</small></Link>)}</div></section>
}
