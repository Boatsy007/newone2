...
type DisplayHighlight = FeaturedHighlight & { clubLogoUrl: string | null }
...
export default function HomeFeaturedHighlights() {
  ...
  if (!target || pathname !== '/' || rows.length === 0) return null
  ...
  return createPortal(
    <section className="pf-featured-highlights pf-shell">
      ...
      <div className="pf-featured-highlights-row">
        {rows.map(row => (
          <article key={row.id} className="pf-featured-highlight-card">
            <Link to={row.detailUrl || `/highlights/${row.id}`} className="pf-featured-highlight-media" aria-label={`Watch ${row.playerName} ${categoryLabel(row.category)}`}>
            ...
            <div className="pf-featured-highlight-club">
              {row.clubId ? <Link to={`/team/${row.clubId}`} aria-label={`Open ${row.clubName} club profile`}><TeamLogo name={row.clubName} src={row.clubLogoUrl ?? undefined} size={58} /></Link> : <TeamLogo name={row.clubName} src={row.clubLogoUrl ?? undefined} size={58} />}
...
.pf-featured-highlight-club{display:flex;align-items:center;gap:14px;padding:16px 20px}.pf-featured-highlight-club>a{display:grid;place-items:center;flex:0 0 62px;width:62px;height:62px;text-decoration:none}.pf-featured-highlight-club img{max-width:58px;max-height:58px;object-fit:contain}.pf-featured-highlight-club div{min-width:0}.pf-featured-highlight-club strong,.pf-featured-highlight-club small{display:block}.pf-featured-highlight-club strong{font-size:17px;color:#111318}.pf-featured-highlight-club small{margin-top:4px;color:#687385;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
