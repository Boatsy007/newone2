...
export default function HomePlayerRecordsPortal() {
  ...
  useEffect(() => {
    if (pathname !== '/') return
    let active = true
    void fetch('/api/goal-kickers/records?limit=10')
      .then(response => response.ok ? response.json() : Promise.reject(new Error(`HTTP ${response.status}`)))
      .then((payload: Payload) => {
        if (!active) return
        setCards(buildRecordCards(payload.data))
      })
...
function buildRecordCards(data: Payload['data']): RecordCard[] {
  if (!data) return []
  ...
  if (seasonLeader) cards.push(toCard('season-leader', 'Season goal leader', seasonLeader.goals, 'goals', seasonLeader))
  if (weeklyLeader?.weeklyGoals) cards.push(toCard('weekly-leader', 'Most goals this week', weeklyLeader.weeklyGoals, weeklyLeader.weeklyGoals === 1 ? 'goal' : 'goals', weeklyLeader))
  if (biggestBag?.weeklyGoals) cards.push(toCard('biggest-bag', 'Biggest recorded bag', biggestBag.weeklyGoals, biggestBag.weeklyGoals === 1 ? 'goal' : 'goals', biggestBag))
  if (rateLeader?.goalsPerGame != null) cards.push(toCard('goals-per-game', 'Goals per game leader', rateLeader.goalsPerGame.toFixed(2), 'per game', rateLeader))
  if (fastest50?.milestoneMatches) cards.push(toCard('fastest-50', 'Fastest to 50 goals', fastest50.milestoneMatches, fastest50.milestoneMatches === 1 ? 'match' : 'matches', fastest50))
  if (fastest100?.milestoneMatches) cards.push(toCard('fastest-100', 'Fastest to 100 goals', fastest100.milestoneMatches, fastest100.milestoneMatches === 1 ? 'match' : 'matches', fastest100))

  if (cards.length < 4) {
    for (const row of data.seasonLeaders?.slice(1) ?? []) {
      if (cards.length >= 6) break
      cards.push(toCard(`season-${row.playerId}-${row.rank}`, `Season goals · #${row.rank}`, row.goals, 'goals', row))
    }
  }
...
function toCard(key: string, kicker: string, value: string | number, unit: string, row: PlayerRecord): RecordCard {
  return {
    key,
    kicker,
    value: String(value),
