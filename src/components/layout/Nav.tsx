...
export default function Nav() {
  ...
  return <>
    ...
    {open && <div className="pf-mobile-menu">
      <nav aria-label="Mobile navigation">{links.map((link, index) => <Link key={link.href} to={link.href}><span>{link.label}</span><small>{String(index + 1).padStart(2, '0')}</small></Link>)}</nav>
    ...
    <style>{`
      ...
      .pf-claim-club-fab{position:fixed;right:18px;bottom:18px;z-index:65;background:#42b8ff;color:#050505;text-decoration:none;text-transform:uppercase;font-family:'Barlow Condensed',Arial,sans-serif;font-size:12px;font-weight:950;letter-spacing:.05em;padding:13px 18px;border-radius:999px;border:1px solid rgba(5,5,5,.14);box-shadow:0 10px 28px rgba(0,0,0,.2)}
      ...
      @media(max-width:480px){.pf-brand img{width:148px}.pf-search{width:36px;height:36px}.pf-nav-inner{gap:8px;padding:0 12px}.pf-claim-club-fab{right:12px;bottom:12px;padding:12px 15px}}
    `}</style>
  </>
}
