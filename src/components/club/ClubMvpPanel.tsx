...
export default function ClubMvpPanel({ clubId, clubName }: { clubId: string; clubName: string }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/mvp?limit=500', { signal: controller.signal })
      ...
      .then(payload => {
        ...
        setRows(all.filter(row => row.clubId === clubId).slice(0, 3))
      })
      .catch(error => {
  ...
  return <section className="club-mvp-panel" aria-label={`${clubName} MVP leaders`}>
    <header>
      <div><span>Club award leaders</span><h2>Club MVP Leaders</h2></div>
      <Link to={`/mvp?clubId=${encodeURIComponent(clubId)}`}>See all →</Link>
    </header>
