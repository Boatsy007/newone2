...
type ResolvedRow = LadderRow & { resolvedClubId: string | null; resolvedLogoUrl: string | null }
...
export default function ConnectedClubLadder({ club }: { club: ClubProfile }) {
  ...
  const ladder = useMemo<ResolvedRow[]>(() => {
    const rows = cleanLadder(Array.isArray(league?.ladder) ? league!.ladder : [])
    return rows.map(row => {
      const isCurrentClub = row.clubId === club.clubId || normaliseName(row.clubName) === clubNameKey
        ...
        <div className="connected-club-ladder-card">
          ...
          {ladder.map(row => {
            ...
            const content = (
              <>
                <b>{row.position}</b>
                <span className="connected-club-ladder-team"><TeamLogo name={row.clubName} src={row.resolvedLogoUrl ?? undefined} size={38} /><strong>{row.clubName}</strong>{isClub && <small>This club</small>}</span>
            ...
            return row.resolvedClubId ? (
              ...
              <div key={rowKey(row)} className={`${className} is-unlinked`} style={style} aria-label={`${row.clubName} ladder row`}>
                {content}
              </div>
            )
          })}
