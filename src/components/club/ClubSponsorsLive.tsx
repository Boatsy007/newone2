...
export default function ClubSponsorsLive({ club }: { club: ClubProfile }) {
  ...
  const majorSponsors = sponsors.filter(row => String(row.tier ?? '').toUpperCase() === 'MAJOR')
  const otherSponsors = sponsors.filter(row => String(row.tier ?? '').toUpperCase() !== 'MAJOR')
  ...
  return (
    <section className="club-sponsors-live">
      ...
      {loading ? (
        ...
        <div className="club-sponsor-groups">
          {majorSponsors.length > 0 && (
            <SponsorCarousel title="Major sponsors" rows={majorSponsors} major />
          )}
          {otherSponsors.length > 0 && (
            <SponsorCarousel title={majorSponsors.length ? 'Club sponsors' : 'Sponsors'} rows={otherSponsors} />
          )}
        </div>
      ) : (
      ...
      <style>{`
        .club-sponsors-live{padding:clamp(30px,5vw,48px);font-family:Barlow,Inter,Arial,sans-serif;overflow:hidden}
        .club-sponsors-head{padding:0 clamp(4px,1vw,10px)}
        ...
        .club-sponsor-groups{display:grid;gap:30px}
        .club-sponsor-group{min-width:0}
        .club-sponsor-group-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin:0 4px 12px}
        ...
        .club-sponsor-carousel{display:flex;gap:14px;overflow-x:auto;overflow-y:hidden;padding:3px 4px 14px;scroll-snap-type:x mandatory;scroll-padding-left:4px;scrollbar-width:none;-webkit-overflow-scrolling:touch;overscroll-behavior-inline:contain}
        .club-sponsor-carousel::-webkit-scrollbar{display:none}
        .club-sponsor-card{flex:0 0 clamp(210px,31vw,310px);min-width:0;min-height:160px;padding:26px;border:1px solid #dfe4e9;border-radius:14px;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:11px;color:#111318;text-align:center;text-decoration:none;box-sizing:border-box;box-shadow:0 7px 20px rgba(17,24,39,.065);scroll-snap-align:start;scroll-snap-stop:always;transition:border-color .18s ease,transform .18s ease}
        .club-sponsor-carousel.major .club-sponsor-card{flex-basis:clamp(300px,48vw,470px);min-height:225px;padding:34px}
        a.club-sponsor-card:hover{border-color:#2daaf5;transform:translateY(-2px)}
        .club-sponsor-card img{display:block;width:auto;max-width:min(250px,82%);height:auto;max-height:92px;object-fit:contain}
        .club-sponsor-carousel.major .club-sponsor-card img{max-width:min(360px,84%);max-height:132px}
        ...
        .club-sponsor-card small{color:#0878bd;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.07em}
        ...
        @media(max-width:620px){
          .club-sponsors-live{padding:28px 20px 34px}
          .club-sponsors-head{padding:0 2px}.club-sponsors-head p{margin-bottom:24px}
          .club-sponsor-groups{gap:27px}
          .club-sponsor-group-head{margin:0 2px 10px}
          .club-sponsor-carousel{gap:10px;margin-left:-2px;margin-right:-20px;padding:3px 20px 13px 2px;scroll-padding-left:2px}
          .club-sponsor-card{flex-basis:calc((100vw - 40px - 20px) / 2.5);min-height:145px;padding:18px 14px;border-radius:12px;gap:8px}
          .club-sponsor-carousel.major .club-sponsor-card{flex-basis:calc((100vw - 40px - 10px) / 1.5);min-height:205px;padding:28px 22px}
          .club-sponsor-card img{max-width:88%;max-height:76px}.club-sponsor-carousel.major .club-sponsor-card img{max-width:88%;max-height:116px}
          .club-sponsor-card strong{font-size:14px}.club-sponsor-carousel.major .club-sponsor-card strong{font-size:19px}
          .club-sponsor-card small{font-size:8px}.club-sponsor-label{font-size:8px}
        }
      `}</style>
    </section>
  )
...
function SponsorCarousel({ title, rows, major = false }: { title: string; rows: Sponsorship[]; major?: boolean }) {
  return (
    <section className="club-sponsor-group">
      <div className="club-sponsor-group-head">
        <h3>{title}</h3>
        {rows.length > 1 && <span>Swipe to explore →</span>}
      </div>
      <div className={`club-sponsor-carousel${major ? ' major' : ''}`}>
        {rows.map(row => {
          ...
          const body = (
            <>
              <span className="club-sponsor-label">{major ? 'Major club partner' : 'Official club sponsor'}</span>
