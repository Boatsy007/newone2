...
export default function PublicClubLiveMatch({clubId}:{clubId:string}){
 ...
 return <section className="public-club-live" aria-label="Live match">
  ...
  <div className="public-club-live-clock">Q{match.quarter} · {formatTime(elapsed)}</div>
  ...
  <footer><span>{match.clockRunning?'Clock running':'Clock paused'}{match.lastEvent?` · ${match.lastEvent}`:''}</span><Link to="/matches">Match Centre →</Link></footer>
  <style>{styles}</style>
 </section>
}
const styles=`.public-club-live{margin:0 0 14px;border:1px solid #f04b55;border-radius:14px;overflow:hidden;background:#0b1118;color:#fff;box-shadow:0 12px 28px rgba(15,23,42,.13)}.public-club-live header,.public-club-live footer{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 14px;background:#121a23;font-size:10px;font-weight:950;text-transform:uppercase}.public-club-live header span{display:flex;align-items:center;gap:8px;color:#ff6872}.public-club-live header i{width:9px;height:9px;border-radius:50%;background:#ef3340;box-shadow:0 0 0 5px rgba(239,51,64,.17);animation:publicClubLivePulse 1.4s infinite}.public-club-live header b{color:#aeb9c4}.public-club-live-clock{padding:9px 14px 0;color:#aeb9c4;font-size:11px;font-weight:950;text-transform:uppercase}.public-club-live-score{display:grid;grid-template-columns:minmax(0,1fr) auto auto auto minmax(0,1fr);align-items:center;gap:12px;padding:14px 14px 18px;text-align:center}.public-club-live-score div:first-child{text-align:right}.public-club-live-score div:last-child{text-align:left}.public-club-live-score strong,.public-club-live-score small{display:block}.public-club-live-score small{margin-top:3px;color:#aeb9c4}.public-club-live-score em{font-family:'Bebas Neue',Impact,sans-serif;font-size:38px;font-style:normal}.public-club-live-score>span{border-radius:999px;padding:6px 8px;background:#ef3340;font-size:10px;font-weight:950}.public-club-live footer span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#b8c2cc}.public-club-live footer a{flex:0 0 auto;color:#8fd5ff;text-decoration:none}@keyframes publicClubLivePulse{50%{box-shadow:0 0 0 10px rgba(239,51,64,0)}}@media(max-width:620px){.public-club-live-score{grid-template-columns:minmax(0,1fr) auto auto auto minmax(0,1fr);gap:7px;padding:12px 9px 15px}.public-club-live-score strong{font-size:12px}.public-club-live-score em{font-size:30px}.public-club-live footer span{display:none}}`
