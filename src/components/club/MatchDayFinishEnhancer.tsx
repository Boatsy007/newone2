...
import { CheckCircle2, Printer, Trophy } from 'lucide-react'
...
export default function MatchDayFinishEnhancer(){
 ...
 return <>{host&&quarterFour&&createPortal(<button className="pf-finish-match" onClick={finishMatch} disabled={busy}><Trophy size={18}/>{busy?'Finishing…':'Finish match'}</button>,host)}{summaryHost&&state?.finishedAt&&createPortal(<GameSummary state={state} opponent={sheet?.opponentName||'Opposition'} clubId={clubId}/>,summaryHost)}{error&&host&&createPortal(<span className="pf-finish-error">{error}</span>,host)}<style>{styles}</style></>
}
