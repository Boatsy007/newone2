...
import { ArrowLeft, CircleDot, Eraser, Maximize2, MousePointer2, Pause, Pencil, Play, Radio, Redo2, RotateCcw, Save, Shapes, Square, Undo2, Users, X } from 'lucide-react'
...
export default function WhiteboardAppMode(){
 ...
 const click=(scope:string,label:string)=>{
  ...
  const target=buttons.find(button=>(button.textContent||'').trim().toLowerCase().startsWith(label.toLowerCase()))
 ...
 const media=(label:string)=>{click('.wb .playbar',label);if(label!=='pause')setTray(null)}
 ...
 const ui=<>
  ...
  {tray==='media'&&<div className="wb-popup wb-media-tray" role="dialog" aria-label="Record and playback controls">
   ...
   <button onClick={()=>media('stop')}><Square/><span>Stop</span></button>
