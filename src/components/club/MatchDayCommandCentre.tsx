...
export default function MatchDayCommandCentre({children}:{children:ReactNode}){
 ...
 async function launch(){
  ...
  // MatchDayFullscreenControl normally learns about fullscreen from the
  // browser event. The command-centre launch uses PlayFooty's stable iPad
  // focus mode, so emit the same event immediately after the workspace mounts.
  document.dispatchEvent(new Event('fullscreenchange'))
  window.dispatchEvent(new CustomEvent('playfooty:matchday-focus',{detail:{active:true}}))

  // Re-sync once the Match Day portals (stats, AI coach, tabs and bench) have
  // completed their first render. This prevents the partially loaded view.
 ...
 if(launched)return <>{children}<style>{focusStyles}</style></>
