...
export default function PublicLiveMatchPortal(){
 ...
 useLayoutEffect(()=>{
  ...
  const connect=()=>{
   const overview=document.querySelector<HTMLElement>('.club-tab-overview .club-profile-main')
   const hero=document.querySelector<HTMLElement>('#main-content > header')
   if(overview){
    if(!liveElement||!liveElement.isConnected){liveElement=document.createElement('div');liveElement.id='playfooty-public-live-match';setHost(liveElement)}
    if(overview.firstElementChild!==liveElement)overview.insertBefore(liveElement,overview.firstElementChild)
   }
   if(hero){
    if(!heroElement||!heroElement.isConnected){heroElement=document.createElement('div');heroElement.id='playfooty-live-match-hero-button';setHeroHost(heroElement)}
    if(heroElement.parentElement!==hero)hero.appendChild(heroElement)
   }
  }
  connect()
...
const styles=`
#playfooty-public-live-match{display:block;scroll-margin-top:75px;margin-bottom:18px}
...
.public-live-match{width:100%;box-sizing:border-box;padding:18px 20px;border-radius:16px;background:linear-gradient(115deg,#071018,#152330);color:#fff;box-shadow:0 12px 28px rgba(15,23,42,.14)}
