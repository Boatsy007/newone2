import { ArrowRight, CalendarCheck, CheckCircle2, ClipboardCheck, Dumbbell, ShieldCheck, Swords, Target } from 'lucide-react'
  ...
  return <section className="coach-command-centre" aria-label="Coaching command centre">
    ...
    {loading ? <div className="coach-command-loading">Loading coaching priorities…</div> : error ? <div className="coach-command-error">{error}</div> : <>
      ...
      <div className="coach-command-status">
        ...
        <Link to={`/club-portal/${clubId}/team-selection`} className={selectionReady ? 'selection-published' : ''}><ShieldCheck size={20}/><span><small>Team selection</small><strong className="coach-selection-value">{selectionReady ? <><CheckCircle2 size={24}/><span>Published</span></> : `${selection?.playerCount ?? 0} selected`}</strong><em>{selectionReady ? 'Team published' : selectionStatus.replaceAll('_', ' ').toLowerCase()}</em></span></Link>
