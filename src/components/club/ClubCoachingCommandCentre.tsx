...
export default function ClubCoachingCommandCentre({ clubId }: { clubId: string }) {
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      test: players.filter(player => player.status === 'TEST' || player.status === 'UNSURE' || player.status === 'UNLIKELY').length,
  ...
  const primary = counts.pending > 0
    ? { icon: CalendarCheck, eyebrow: 'Needs attention', title: `${counts.pending} player${counts.pending === 1 ? '' : 's'} yet to respond`, copy: 'Check availability before finalising this week’s side.', href: `/club-portal/${clubId}/availability`, action: 'Review availability' }
    : !selectionReady
      ...
      : { icon: Swords, eyebrow: 'Match ready', title: 'Prepare the match-day workspace', copy: `${selection?.playerCount ?? 0} players are selected and the team is ready for match day.`, href: `/club-portal/${clubId}/coaching?view=match-day`, action: 'Open Match Day' }
  const PrimaryIcon = primary.icon

  return createPortal(<section className="coach-command-centre" aria-label="Coaching command centre">
    <div className="coach-command-heading">
      <div><span>COACHING COMMAND CENTRE</span><h2>This week</h2><p>Your selection, availability and match preparation in one place.</p></div>
      <Link to={`/club-portal/${clubId}/coaching?view=team`}>Open squad <ArrowRight size={15}/></Link>
    </div>

    {loading ? <div className="coach-command-loading">Loading coaching priorities…</div> : <>
      ...
      <div className="coach-command-actions">
        <Link to={`/club-portal/${clubId}/coaching?view=training`}><Dumbbell size={19}/><span>Training</span></Link>
        <Link to={`/club-portal/${clubId}/whiteboard`}><Workflow size={19}/><span>Whiteboard</span></Link>
        <Link to={`/club-portal/${clubId}/coaching?view=player-development`}><Sparkles size={19}/><span>Development</span></Link>
        <Link to={`/club-portal/${clubId}/users`}><Users size={19}/><span>Coaching access</span></Link>
      </div>
    </>}
    <style>{styles}</style>
  </section>, host)
