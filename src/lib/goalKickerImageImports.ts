...
export const goalKickerImageImports={
 ...
 commit:(body:{leagueId:string;season:string;grade:string;rows:Array<{playerName:string;clubName:string;clubId:string|null;goals:number;matches:number|null}>})=>request<{data:{imported:number;errors:number;league:string;season:string;grade:string};errors?:unknown[]}>('/admin/goal-kicker-images/commit',body),
}
