...
export function AppShell({club,session,onSwitchClub,onSignOut}:Props){
 ...
 function changeArea(next:AppArea){setTrainingSession(null);setReportSession(null);setArea(next)}
 return <View style={styles.app}><Sidebar active={area} club={club} onChange={changeArea} onSwitchClub={onSwitchClub}/><View style={styles.main}>{area==='overview'?<OverviewScreen club={club} onSignOut={onSignOut}/>:area==='coaching'&&trainingSession?<TrainingPlanScreen club={club} session={session} sessionNumber={trainingSession} onBack={()=>setTrainingSession(null)}/>:area==='coaching'&&reportSession?<TrainingReportScreen club={club} session={session} sessionNumber={reportSession} onBack={()=>setReportSession(null)}/>:area==='coaching'?<CoachingScreen club={club} session={session} onOpenTrainingPlan={setTrainingSession} onOpenTrainingReport={setReportSession}/>:<View style={styles.coming}><Text style={styles.eyebrow}>PLAYFOOTY CLUB APP</Text><Text style={styles.title}>{area}</Text><Text style={styles.copy}>This completed PlayFooty module will be connected in its dedicated build stage.</Text></View>}</View></View>
}
