...
export type ParsedTeamGoalKickers = {
  teamName: string | null
  goalKickers: MatchDetailKicker[]
  confidence: number | null
  notes: string | null
}
...
async function visionJson(image: string, prompt: string, maxTokens: number): Promise<Record<string, unknown>> {
  ...
  const response = await fetch(API, {
    ...
    body: JSON.stringify({ model, max_tokens: maxTokens, messages: [{ role: 'user', content: [{ type: 'image', source: { type: 'base64', media_type: src.media_type, data: src.data } }, { type: 'text', text: prompt }] }] }),
...
- confidence must be between 0 and 1 and reflect the overall extraction quality.`, 3500)
...
- confidence must be between 0 and 1.`, 2200)
  const confidenceValue = Number(parsed.confidence)
  return {
    teamName: parsed.teamName == null ? null : String(parsed.teamName).trim() || null,
    goalKickers: cleanKickers(parsed.goalKickers),
    confidence: Number.isFinite(confidenceValue) ? Math.max(0, Math.min(1, confidenceValue)) : null,
    notes: parsed.notes == null ? null : String(parsed.notes).trim() || null,
  }
}
