...
async function ownedDeal(clubId:string,id:string){
  return prisma.sponsorship.findFirst({where:{id,clubId,deletedAt:null},include:{sponsor:true}})
}
...
router.get('/clubs/:clubId/sponsors',async(req,res)=>{
  try{
    const rows=await prisma.sponsorship.findMany({where:{clubId:req.params.clubId,deletedAt:null},include:{sponsor:true},orderBy:[{displayPriority:'desc'},{createdAt:'desc'}]})
    res.json({data:rows})
  }catch(error){res.status(500).json({error:'Unable to load club sponsors',detail:String(error)})}
})

router.post('/clubs/:clubId/sponsors',async(req,res)=>{
  try{
    const club=await prisma.club.findFirst({where:{id:req.params.clubId,archivedAt:null},select:{id:true,name:true,state:{select:{code:true}}}})
    ...
    const created=await createSponsor({name,businessName:String(body.businessName??name),websiteUrl:body.websiteUrl||null,email:body.email||null,phone:body.phone||null,description:body.description||null,industry:body.industry||null,state:club.state.code,tier:body.tier||'CLUB',status:'PENDING'},actor)
    if(!created.ok)return res.status(400).json({error:created.error})
    const deal=await createSponsorship({sponsorId:created.sponsor.id,scope:'CLUB',clubId:club.id,package:body.package||'CLUB_PARTNER',tier:body.tier||'CLUB',startDate:body.startDate||null,endDate:body.endDate||null,displayPriority:Number(body.displayPriority??0),bannerPosition:body.bannerPosition||'CLUB_PROFILE',ctaLabel:body.ctaLabel||'Visit sponsor',ctaUrl:body.ctaUrl||body.websiteUrl||null,notes:body.notes||null,currency:'AUD'},actor)
  ...
  }catch(error){res.status(500).json({error:'Unable to create sponsor',detail:String(error)})}
})

router.patch('/clubs/:clubId/sponsors/:id',async(req,res)=>{
  try{
    const deal=await ownedDeal(req.params.clubId,req.params.id)
    if(!deal)return res.status(404).json({error:'Sponsorship not found'})
    if(!EDITABLE_STATUSES.includes(deal.status))return res.status(409).json({error:'Approved or active sponsorships must be changed by PlayFooty Admin'})
    const body=(req.body??{}) as Record<string,unknown>
    const actor=`club:${req.clubUser!.id}`
    const sponsorFields:Record<string,unknown>={}
    for(const key of ['name','businessName','websiteUrl','email','phone','description','industry','facebookUrl','instagramUrl','brandPrimary','brandSecondary'])if(key in body)sponsorFields[key]=body[key]||null
    ...
    for(const key of ['package','tier','bannerPosition','ctaLabel','ctaUrl','notes'])if(key in body)updates[key]=body[key]||null
    ...
    const saved=Object.keys(updates).length?await prisma.sponsorship.update({where:{id:deal.id},data:updates}):deal
  ...
  }catch(error){res.status(500).json({error:'Unable to update sponsor',detail:String(error)})}
})

router.post('/clubs/:clubId/sponsors/:id/logo',async(req,res)=>{
  try{
    const deal=await ownedDeal(req.params.clubId,req.params.id)
    if(!deal)return res.status(404).json({error:'Sponsorship not found'})
    if(!EDITABLE_STATUSES.includes(deal.status))return res.status(409).json({error:'Approved or active sponsor logos must be changed by PlayFooty Admin'})
    const body=(req.body??{}) as Record<string,unknown>,contentType=String(body.contentType??'image/png')
    if(!IMAGE_TYPES.has(contentType))return res.status(400).json({error:'Use PNG, JPG, WEBP or SVG'})
    const raw=String(body.dataUrl??body.base64??''),base64=raw.includes(',')?raw.split(',').pop()!:raw,buffer=Buffer.from(base64,'base64')
    if(!buffer.length||buffer.length>5*1024*1024)return res.status(400).json({error:'Sponsor logo must be between 1 byte and 5 MB'})
    ...
    const upload=await fetch(`${url}/storage/v1/object/${LOGO_BUCKET}/${path}`,{method:'POST',headers:{authorization:`Bearer ${key}`,apikey:key,'content-type':contentType,'x-upsert':'true'},body:buffer})
    ...
    const sponsor=await prisma.commercialSponsor.update({where:{id:deal.sponsorId},data:{logoUrl,squareLogoUrl:logoUrl}})
...
router.delete('/clubs/:clubId/sponsors/:id',async(req,res)=>{
  try{
    const deal=await ownedDeal(req.params.clubId,req.params.id)
    if(!deal)return res.status(404).json({error:'Sponsorship not found'})
    if(!EDITABLE_STATUSES.includes(deal.status))return res.status(409).json({error:'Approved or active sponsorships must be archived by PlayFooty Admin'})
  ...
  }catch(error){res.status(500).json({error:'Unable to archive sponsor',detail:String(error)})}
})
