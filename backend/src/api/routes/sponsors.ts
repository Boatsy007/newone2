/* Sponsorship & commercial public API. */
...
import { createSponsorship, transition, softDeleteSponsorship, getClubSponsorships, getLeagueSponsorships, getPlayerSponsorships, type SponsorshipStatus } from '../../commercial/sponsorships.service.js'
...
async function uploadSponsorLogo(sponsorId: string, body: Record<string, unknown>) {
  ...
  const upload = await fetch(`${supabaseUrl.replace(/\/$/, '')}/storage/v1/object/${LOGO_BUCKET}/${path}`, { method: 'POST', headers: { authorization: `Bearer ${serviceKey}`, apikey: serviceKey, 'content-type': contentType, 'x-upsert': 'true' }, body: buffer })
...
sponsors.get('/', publicRateLimit, cachePublic(300), async (req, res) => res.json({ data: await listSponsors({ state: req.query.state as string | undefined, industry: req.query.industry as string | undefined, status: req.query.status as string | undefined }) }))
sponsors.get('/:id', publicRateLimit, cachePublic(300), async (req, res) => { const sponsor = await getSponsor(String(req.params.id)); if (!sponsor) return res.status(404).json({ error: 'sponsor not found' }); res.json({ data: sponsor }) })
...
commercial.post('/sponsorships', requireAdminActor, async (req, res) => { const result = await createSponsorship((req.body ?? {}) as Record<string, unknown>, 'admin'); res.status(result.ok ? 201 : 400).json(result.ok ? { data: result.sponsorship } : { error: result.error }) })
commercial.patch('/sponsorships/:id', requireAdminActor, async (req, res) => { const body = (req.body ?? {}) as { status?: SponsorshipStatus; note?: string }; if (!body.status) return res.status(400).json({ error: 'status required' }); const result = await transition(String(req.params.id), body.status, { note: body.note, performedBy: 'admin' }); res.status(result.status).json(result.ok ? { data: result.sponsorship } : { error: result.error }) })
commercial.delete('/sponsorships/:id', requireAdminActor, async (req, res) => { const result = await softDeleteSponsorship(String(req.params.id), 'admin'); res.status(result.ok ? 200 : 404).json(result.ok ? { data: { archived: true } } : { error: result.error }) })
commercial.get('/inventory', publicRateLimit, cachePublic(300), async (req, res) => res.json({ data: await listInventory({ placement: req.query.placement as string | undefined, availableOnly: req.query.available === 'true' }) }))
commercial.post('/inventory', requireAdminActor, async (req, res) => { const result = await createInventory((req.body ?? {}) as Record<string, unknown>, 'admin'); res.status(result.ok ? 201 : 400).json(result.ok ? { data: result.inventory } : { error: result.error }) })
commercial.post('/inventory/:id/book', requireAdminActor, async (req, res) => { const body = (req.body ?? {}) as { sponsorshipId?: string; start?: string; end?: string }; if (!body.sponsorshipId) return res.status(400).json({ error: 'sponsorshipId required' }); const result = await bookInventory(String(req.params.id), body.sponsorshipId, { start: body.start, end: body.end, performedBy: 'admin' }); res.status(result.ok ? 200 : 400).json(result.ok ? { data: result.inventory } : { error: result.error }) })
commercial.post('/inventory/:id/release', requireAdminActor, async (req, res) => { const result = await releaseInventory(String(req.params.id), 'admin'); res.status(result.ok ? 200 : 404).json(result.ok ? { data: result.inventory } : { error: result.error }) })
commercial.get('/tiers', publicRateLimit, cachePublic(3600), async (_req, res) => res.json({ data: await listTiers() }))
commercial.post('/tiers', requireAdminActor, async (req, res) => { const result = await addCustomTier((req.body ?? {}) as Record<string, unknown>, 'admin'); res.status(result.ok ? 201 : 400).json(result.ok ? { data: result.tier } : { error: result.error }) })
commercial.get('/premium', requireAdminActor, async (req, res) => res.json({ data: await listPremium({ scope: req.query.scope as 'CLUB' | 'LEAGUE' | undefined, status: req.query.status as string | undefined }) }))
commercial.get('/premium/:scope/:entityId', publicRateLimit, cachePublic(300), async (req, res) => { const scope = String(req.params.scope).toUpperCase(); if (scope !== 'CLUB' && scope !== 'LEAGUE') return res.status(400).json({ error: 'scope must be club|league' }); res.json({ data: await getPremium(scope, String(req.params.entityId)) }) })
commercial.post('/premium/:scope/:entityId', requireAdminActor, async (req, res) => { const scope = String(req.params.scope).toUpperCase(); if (scope !== 'CLUB' && scope !== 'LEAGUE') return res.status(400).json({ error: 'scope must be club|league' }); res.json({ data: await setPremium(scope, String(req.params.entityId), (req.body ?? {}) as Record<string, unknown>, 'admin') }) })
commercial.post('/sweep', requireAdminActor, async (req, res) => { try { res.json({ data: await runCommercialSweep({ seed: (req.body as { seed?: boolean })?.seed, performedBy: 'admin' }) }) } catch (error) { logger.error('commercial sweep', { detail: String(error) }); res.status(500).json({ error: 'sweep failed' }) } })

const commercialClub = Router()
commercialClub.get('/:id/sponsors', publicRateLimit, cachePublic(300), async (req, res) => res.json({ data: await getClubSponsorships(String(req.params.id), req.query.all !== 'true') }))
