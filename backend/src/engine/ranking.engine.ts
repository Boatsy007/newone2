...
function scoreRecentForm(form: MatchResult[]): number {
  ...
  const weights = form.map((_, i) => i + 1)
...
/** Consistency — high win rate plus positive average margin. */
...
export class RankingEngine {
  ...
  /** Rank a full cohort. All normalisation is relative to this cohort. */
  rankCohort(
    ...
    // A club must always retain the power rating produced from its own component
    // scores. Never transfer or swap ratings between clubs to enforce an order.
    // That made a poor-record club capable of inheriting a top club's rating.
    scored.sort((a, b) => {
