...
router.patch('/:id', async (req, res) => {
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; weekKey: string; category: string }>>(`SELECT id, week_key AS "weekKey", category FROM highlight_submissions WHERE id = $1 LIMIT 1`, req.params.id)
  ...
  await prisma.$executeRawUnsafe(`
    UPDATE highlight_submissions SET
      ...
      voting_opens_at = CASE WHEN $5::boolean THEN $6 ELSE voting_opens_at END,
      voting_closes_at = CASE WHEN $7::boolean THEN $8 ELSE voting_closes_at END,
  ...
  `, req.params.id, status || null, 'moderationNote' in body ? clean(body.moderationNote, 1000) || null : null,
    clean(body.weekKey, 20) || null, 'votingOpensAt' in body, opens, 'votingClosesAt' in body, closes)
...
router.post('/:id/winner', async (req, res) => {
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; weekKey: string; category: string }>>(`SELECT id, week_key AS "weekKey", category FROM highlight_submissions WHERE id = $1 AND status = 'APPROVED' LIMIT 1`, req.params.id)
  const target = rows[0]
  if (!target) return res.status(404).json({ error: 'Approved nominee not found' })
  await prisma.$transaction([
    prisma.$executeRawUnsafe(`UPDATE highlight_submissions SET winner = FALSE, updated_at = NOW() WHERE week_key = $1 AND category = $2`, target.weekKey, target.category),
    prisma.$executeRawUnsafe(`UPDATE highlight_submissions SET winner = TRUE, updated_at = NOW() WHERE id = $1`, target.id),
  ])
  res.json({ data: { id: target.id, winner: true } })
})

router.delete('/:id', async (req, res) => {
  await prisma.$executeRawUnsafe(`DELETE FROM highlight_submissions WHERE id = $1`, req.params.id)
