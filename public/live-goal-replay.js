(() => {
  ...
  style.textContent = `
    ...
    .pf-goal-replay.show{transform:translateX(0);opacity:1;pointer-events:auto}.pf-goal-replay.exit{transform:translateX(-105%);opacity:0}.pf-goal-replay video{display:block;width:100%;height:100%;object-fit:contain;background:#000}
    ...
    @media(max-width:600px){.pf-goal-replay-label{right:8px;top:8px;padding:7px 10px;font-size:10px}.pf-goal-replay-title{left:8px;right:8px;bottom:8px;padding:9px 11px;font-size:11px}.pf-live-timeline-head h2{font-size:21px}}
    @media(prefers-reduced-motion:reduce){.pf-goal-replay{transition:none}.pf-goal-replay-label,.pf-live-event{animation:none}}
  `
  ...
  const timelineList = timeline.querySelector('.pf-live-timeline-list')
