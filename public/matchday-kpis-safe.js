(() => {
  ...
  function style() {
    ...
    node.textContent = `
      ...
      #pf-kpi-tab{background:#d5a112!important;color:#101820!important}
      .md>.pf-kpi-drawer,.md:fullscreen>.pf-kpi-drawer,.md:-webkit-full-screen>.pf-kpi-drawer,body.pf-match-day-focus .md>.pf-kpi-drawer{position:absolute!important;z-index:124!important;left:0!important;top:0!important;right:auto!important;bottom:auto!important;width:50vw!important;min-width:50vw!important;max-width:50vw!important;height:100vh!important;min-height:100vh!important;max-height:100vh!important;box-sizing:border-box!important;padding:18px!important;background:#07111c!important;color:#fff!important;border-right:1px solid #263946!important;border-radius:0 18px 18px 0!important;box-shadow:18px 0 45px rgba(0,0,0,.5)!important;transform:translateX(-102%)!important;transition:transform .2s ease!important;overflow:auto!important}.md>.pf-kpi-drawer.open,.md:fullscreen>.pf-kpi-drawer.open,.md:-webkit-full-screen>.pf-kpi-drawer.open,body.pf-match-day-focus .md>.pf-kpi-drawer.open{transform:translateX(0)!important}.pf-kpi-drawer-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.pf-kpi-drawer-head h2{margin:0}.pf-kpi-close{width:40px;height:40px;border:1px solid #334754;border-radius:9px;background:#13232e;color:#fff;font-size:22px}.pf-kpi-drawer .pf-kpi-row{border-color:#263946;background:#0e1b25}.pf-kpi-drawer .pf-kpi-label small{color:#8798a4}.pf-kpi-drawer .pf-kpi-value{border-color:#3a4d59;background:#07111c;color:#fff}
      @media(max-height:620px){.md>.pf-kpi-drawer,.md:fullscreen>.pf-kpi-drawer,.md:-webkit-full-screen>.pf-kpi-drawer,body.pf-match-day-focus .md>.pf-kpi-drawer{padding:11px 14px!important}.pf-kpi-drawer .pf-kpi-row{padding:5px 8px;grid-template-columns:minmax(0,1fr) 40px 50px 40px}.pf-kpi-drawer .pf-kpi-step,.pf-kpi-drawer .pf-kpi-value{height:36px}}
    `
  ...
  function findGamePlanTab(board) {
    return [...board.querySelectorAll('button')].find(button => button.textContent?.trim().toLowerCase() === 'game plan') || null
  }
  ...
  function mountMatchDay() {
    ...
    const gamePlanTab = findGamePlanTab(board)
    if (!gamePlanTab || !gamePlanTab.parentElement) return

    const targets = load()
    const tab = gamePlanTab.cloneNode(false)
    tab.id = 'pf-kpi-tab'
    tab.type = 'button'
  ...
  function unmountMatchDay() {
    document.getElementById('pf-kpi-tab')?.remove()
    document.getElementById('pf-kpi-drawer')?.remove()
    open = false
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (step) {
      ...
      const targets = load()
      const stat = step.getAttribute('data-kpi-stat')
      const delta = Number(step.getAttribute('data-kpi-delta') || 0)
      if (stat && delta) {
        targets[stat] = Math.max(0, Number(targets[stat] || 0) + delta)
        save(targets)
        document.querySelectorAll(`[data-kpi-value="${stat}"]`).forEach(value => value.textContent = String(targets[stat]))
        applyColours()
      }
      return
    }
    if (target?.closest('#pf-kpi-tab')) {
      open = !open
      document.getElementById('pf-kpi-drawer')?.classList.toggle('open', open)
      return
    }
    if (target?.closest('.pf-kpi-close')) {
      open = false
      document.getElementById('pf-kpi-drawer')?.classList.remove('open')
      return
    }
